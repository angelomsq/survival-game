...
export default class Character extends Phaser.Physics.Arcade.Sprite {
  ...
  setCollisionBody(width, height) {
    let bodyWidth = Math.round(this.w * 0.7)
    // console.log('body W:',bodyW);
    let bodyHeight = Math.round(this.h * 0.3)
  ...
  addHurtBox() {
    this.hurtbox = this.scene.add
      .rectangle(this.x, this.y, this.w, this.h, 0x6666ff)
      .setVisible(false)
  ...
  updateHurtBox(width, height) {
    this.hurtbox.setPosition(this.x, this.y)
    let bodyWidth = Math.round(width * 0.8)
  ...
  addShadow() {
    this.shadow = this.scene.add.ellipse(
      this.x,
      this.y + this.h / 2,
      Math.round(this.w * 0.7),
      Math.round(this.h * 0.3),
      0x222222,
      0.6
    )
  }
  ...
  updateShadow(width, height) {
    let bodyWidth = Math.round(width * 0.7)
    let bodyHeight = Math.round(height * 0.3)
    let bottomCenter = this.getBottomCenter()
    // this.shadow.setSize(bodyWidth, bodyHeight)
    this.shadow.setDisplaySize(bodyWidth, bodyHeight)
    this.shadow.setPosition(bottomCenter.x, bottomCenter.y)
  }
